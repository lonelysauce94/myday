...
- 用户电脑上的本地克隆只是 `main` 的一份镜像，不要假设本地有未推送的改动。
- **如果当前会话跑在用户电脑的本地克隆里（而不是云端新鲜克隆）：开工第一件事先 `git fetch origin && git status`，落后了就 `git pull origin main` 把文件夹同步到最新，再动手改。** 克隆自带 origin 指向 GitHub，不需要重新配置链接；改完照常推 `main` 上线，这样本地文件夹推完即与线上一致。
...
- **`state.savedAt` 是云同步唯一的胜负判据，只有用户的真实改动才能动它。** 启动阶段的整理（`rolloverTasks` / `migrate*`）调 `save()` 时只落本地、不盖时间戳、不上传——否则一台好几天没开的设备一开机顺延一下，旧数据就会冒充"最新"把另一台刚写的整个覆盖掉（2026-07-24 手机记录丢失就是这样，见该次提交）。
- 云端数据丢了可以从 Gist 修订历史捞回来：`GET /gists/{id}` 的 `history[]` 给出各版本 sha 和时间，`GET /gists/{id}/{完整sha}` 取那一版内容（sha 必须用全长）。历史大约只保留最近 30 版。
